...
class TodoPolicy:
    ...
    def normalize(cls, todo: "PlannedTodo", state: "RunState") -> "PlannedTodo":
        ...
        cls._normalize_durable_artifact_references(todo, state, context)
    ...
    @classmethod
    def _normalize_durable_artifact_references(
        cls,
        todo: "PlannedTodo",
        state: "RunState",
        context: dict[str, Any],
    ...
    def _rewrite_files_root_artifact_paths(
        ...
        pattern = re.compile(re.escape(files_root) + r"/[^\s'\"`<>|&;]+")
    ...
    def _referenced_artifact_directory_prefixes(
        ...
        for match in pattern.finditer(text):
            ...
            if not rel or rel.startswith(".autopentest_artifacts/"):
                continue
            if cls._artifact_by_relative_path(state, rel) is not None:
                continue
            if not cls._durable_artifact_directory_for_relative_prefix(state, rel):
                continue
            if rel not in seen:
                seen.add(rel)
                prefixes.append(rel)
        ...
        *,
        ...
        for value in cls._iter_context_strings(context):
            ...
            if not rel:
                continue
            candidates = [rel]
            candidates.extend(f"{prefix.rstrip('/')}/{rel.lstrip('./')}" for prefix in base_prefixes)
            for candidate in candidates:
                artifact = cls._artifact_by_relative_path(state, candidate)
                if artifact is None:
                    continue
    ...
    def _iter_context_strings(value: Any) -> list[str]:
        ...
        if isinstance(value, str):
            ...
            for item in value.values():
                out.extend(TodoPolicy._iter_context_strings(item))
    ...
    def _durable_artifact_path_for_files_root_path(
        ...
        if not rel or rel.startswith(".autopentest_artifacts/"):
            return None
        artifact = cls._artifact_by_relative_path(state, rel)
        if artifact is not None:
            return str(getattr(artifact, "path", "") or "").strip() or None
        return cls._durable_artifact_directory_for_relative_prefix(state, rel)
    ...
    @staticmethod
    def _artifact_relative_path(artifact: Any) -> str:
    ...
    @classmethod
    def _artifact_by_relative_path(cls, state: "RunState", relative_path: str) -> Any | None:
        target = relative_path.strip().strip("/")
        if not target:
            return None
        for artifact in state.artifacts.values():
            if cls._artifact_relative_path(artifact) == target:
                return artifact
        return None

    @classmethod
    def _durable_artifact_directory_for_relative_prefix(
        cls,
        state: "RunState",
        relative_prefix: str,
    ) -> str | None:
        ...
        for artifact in state.artifacts.values():
            rel = cls._artifact_relative_path(artifact)
            path = str(getattr(artifact, "path", "") or "").strip()
            if not rel.startswith(prefix_with_sep) or not path.endswith(rel):
                continue
            return path[: -len(rel)] + prefix
        return None
    ...
    def _normalize_artifact_target_context(
        ...
        if path:
            ...
            if artifact is not None:
                cls._bind_artifact_target(context, artifact)
