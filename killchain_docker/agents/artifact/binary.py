...
class BinaryTriageAgent(WorkerAgent):
    ...
    def run(self, task: Task, state: GlobalState) -> WorkerReport:
        if task.task_type == "artifact.binary_disassembly":
            ...
            report = run_simple_review(
                ...
                role_addition=(
                    ...
                    "infer the algorithm before writing a solver; do NOT guess "
        ...
        if task.task_type == "artifact.binary_run":
            ...
            # leave headroom.  The plugin script handles arg generation,
    ...
    def _maybe_queue_binary_run_followup(
        ...
        report.new_tasks.append(followup)
        report.notes.append(
            f"{self.name} queued sandboxed binary_run follow-up "
    ...
    def _run_binary_execution(self, task: Task, state: GlobalState) -> WorkerReport:
        """Run the ``binary_run`` plugin to probe binary behaviour at runtime."""
        ...
        new_tasks = build_flag_validation_tasks(flag_candidates, source="binary_run")
        ...
        return success_report(
            ...
            error=(
                None
                if inspected
                else "binary_run: 0 binaries actually executed; check binary_files context."
            ),
        )
    ...
    def _maybe_queue_disassembly_followup(
        ...
        report.new_tasks.append(followup)
        report.notes.append(
            f"{self.name} queued deep disassembly follow-up "
