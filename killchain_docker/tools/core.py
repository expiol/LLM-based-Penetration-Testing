...
class ToolInterruptBehavior(StrEnum):
    CANCEL = "cancel"
    CONTINUE = "continue"
    TERMINAL = "terminal"
...
class ToolExecutionPolicy(BaseModel):
    """Runtime safety metadata for one tool call."""
...
ToolOutputBuilder = Callable[
    [ToolExecutionRequest, ToolExecutionResult, ParsedToolOutput], ToolOutput
]
...
class ExecutionPlane:
    ...
    def execute(
        self, task_id: str, request: ToolExecutionRequest
    ) -> ToolExecutionBundle:
