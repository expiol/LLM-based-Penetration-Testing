...
_PATH_RE = re.compile("(/[a-zA-Z0-9_./-]{4,})")
        ...
        return _run(
            ...
            [
                *self.argv_prefix,
                "bash",
                "-c",
                f"strings -n {min_len} -e {encoding} {path}",
            ],
            request.timeout_s,
        )
...
def build_output(
    ...
    for line_s in interesting:
        m = re.search(
            "(?:password|passwd|secret|token)\\s*[:=]\\s*(\\S+)", line_s, re.IGNORECASE
        )
        if m:
            credentials.append(
                Credential(
    ...
    output_context: dict[str, Any] = {"path": path, "string_count": len(lines)}
    if urls:
