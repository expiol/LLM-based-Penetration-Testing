...
from killchain_docker.state import Endpoint, Vulnerability
...
# nikto findings start with "+ " and contain OSVDB or description
_FINDING_RE = re.compile(r"^\+\s+(.+)$", re.MULTILINE)

# Server header from nikto output
_SERVER_RE = re.compile(r"^\+\s+Server:\s*(.+)", re.MULTILINE | re.IGNORECASE)

# Target IP/hostname from nikto output
_TARGET_IP_RE = re.compile(r"^\+\s+Target IP:\s*(.+)", re.MULTILINE)

# OSVDB reference
_OSVDB_RE = re.compile(r"OSVDB-(\d+)", re.IGNORECASE)

# Severity heuristics based on nikto finding text
_HIGH_KEYWORDS = frozenset({
    "remote code execution", "rce", "command injection", "sql injection",
    "file inclusion", "lfi", "rfi", "backdoor", "root", "admin password",
    "directory traversal", "path traversal", "shell",
})
_MEDIUM_KEYWORDS = frozenset({
    "xss", "cross-site", "csrf", "clickjacking", "information disclosure",
    "directory listing", "source code", "backup", "default credential",
    "phpinfo", ".git", ".svn", ".env",
})
...
class NiktoPlugin:
    ...
    def execute(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        parts: list[str] = [
            "nikto",
            "-h", target,
            f"-maxtime", f"{request.timeout_s}s",
        ]

        # Cookie: explicit or from curl session jar
        if cookie:
            # nikto -C expects "name=value; name2=value2"
            parts.extend(["-C", cookie])
        elif session_id:
            safe_id = re.sub(r"[^a-zA-Z0-9_-]", "", session_id)[:64] or "default"
            jar_path = f"{_JAR_DIR}/{safe_id}.cookies"
            # Read cookie jar and convert to nikto format via shell
            # Netscape cookie jar format: domain\tTRUE\tpath\tFALSE\texpiry\tname\tvalue
            cookie_cmd = (
                f"cookies=$(awk -F'\\t' '!/^#/ && NF>=7 {{printf \"%s=%s; \",$6,$7}}' "
                f"{_shell_quote(jar_path)} 2>/dev/null); "
            )
...
def build_output(
    ...
    # -- Parse findings ------------------------------------------------------
    finding_lines = [line.strip() for line in stdout.splitlines() if line.strip().startswith("+")]

    # Separate informational lines from actual findings
    info_lines: list[str] = []
    vuln_lines: list[str] = []
    for line in finding_lines:
        content = line.lstrip("+ ").strip()
        # Informational: server, target, port, etc.
        if any(content.lower().startswith(prefix) for prefix in (
            "server:", "target", "start time", "end time",
            "host(s) tested", "statistics", "scan terminated",
        )):
