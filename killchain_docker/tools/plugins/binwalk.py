...
class BinwalkPlugin:
    ...
    def execute(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        if extract:
            max_mb = _positive_int(request.metadata.get("max_extract_mb"), _DEFAULT_EXTRACT_MB)
...
def _bounded_extract_command(quoted_path: str, *, max_mb: int) -> str:
    ...
    return (
        '_kc_bw_out="$CTF_TEMP_DIR/binwalk_out"; '
        'mkdir -p "$_kc_bw_out"; '
        f"binwalk -e --run-as=root --directory=\"$_kc_bw_out\" {quoted_path} & "
        '_kc_bw_pid=$!; _kc_bw_limited=0; '
        'while kill -0 "$_kc_bw_pid" 2>/dev/null; do '
        '_kc_bw_kb=$(du -sk "$_kc_bw_out" 2>/dev/null | awk \'{print $1 + 0}\'); '
        f'if [ "$_kc_bw_kb" -gt {budget_kb} ]; then '
        f'echo "[binwalk extraction budget exceeded: ${{_kc_bw_kb}}KB > {budget_kb}KB]" >&2; '
        'kill -TERM "$_kc_bw_pid" 2>/dev/null || true; '
        'sleep 1; '
        'kill -KILL "$_kc_bw_pid" 2>/dev/null || true; '
        '_kc_bw_limited=1; '
        'break; '
        'fi; '
        'sleep 1; '
        'done; '
        'wait "$_kc_bw_pid"; _kc_bw_rc=$?; '
        'if [ "$_kc_bw_limited" = 1 ]; then _kc_bw_rc=0; fi; '
        'find "$_kc_bw_out" -type f -printf "%p\\t%s\\n" 2>/dev/null '
        '| sort -k2,2nr | head -50; '
        'exit "$_kc_bw_rc"'
    )
...
def build_output(
    ...
    # Extracted files live in the tool scratch workspace and are removed after
    # execution. Keep them in evidence for planning, but do not register them
    # as durable state artifacts.
