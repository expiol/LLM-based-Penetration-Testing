...
_PW_RE = re.compile(r"(?:PASSWORD FOUND|pw\s*==)\s*[\"']?(.+?)[\"']?\s*$", re.IGNORECASE | re.MULTILINE)
...
class FcrackzipPlugin:
    ...
    def execute(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        wordlist = str(request.metadata.get("wordlist") or "/usr/share/wordlists/rockyou.txt")
        extra = str(request.metadata.get("extra_args") or "")
        # Crack, then if successful auto-extract to /tmp for inspection
        cmd = (
            f"fcrackzip -u -D -p {wordlist} {extra} {path} && "
            f"pw=$(fcrackzip -u -D -p {wordlist} {path} 2>/dev/null | "
            f"grep -ioP '(?:pw ==|PASSWORD FOUND)\\s*\\K\\S+'); "
            f'[ -n "$pw" ] && unzip -o -P "$pw" {path} -d /tmp/fcrackzip_out 2>&1 || true'
        )
...
def build_output(
    ...
    # -- Parse extracted files (from unzip output) ---------------------------
