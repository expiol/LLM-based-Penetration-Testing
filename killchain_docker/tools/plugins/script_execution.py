"""Execute LLM-generated scripts inside the agent container."""
...
# ``flag{}`` matches that may appear in source code echoed back from scripts
# (e.g. ``re.findall(r'flag\{...}')`` literals).
...
# since scripts may print debug logs / tracebacks / source echoes there).
...
# Body executes the user script and harvests flag candidates from its output:
#
...
#    ``"with open('x') as f:"``.  Stderr is never scanned because scripts
#    print debug logs / tracebacks / source echoes there.
_SCRIPT_BODY = r"""
def _looks_like_script_source(text):
    # Return True for lines that are clearly echoed source code, not flags.
    needles = (
        # Calls into common Python idioms most scripts use:
...
def _record_candidate(match, bucket):
    ...
    if not _plausible_flag(match):
        return
    if _looks_like_script_source(match):
        return
    bucket.append(match)
...
suffix = suffix_map.get(script_language, ".py")
interpreter = interpreter_map.get(script_language, ["python3"])

with tempfile.NamedTemporaryFile(
    mode="w", suffix=suffix, dir=str(files_root), delete=False, prefix="_script_"
) as tmp:
...
try:
    for raw in challenge_files:
        ...
        # the script overwrites it (e.g. running ``stfu flag.stfu`` in-place
    ...
    stdout = ""
    stderr = f"Script execution error: {type(exc).__name__}: {exc}"
    returncode = -1
    notes_list.append(stderr)
    ...
    for path_str, _orig_mode, recorded_digest, snapshot_bytes in _challenge_snapshots:
        ...
        try:
            ...
            notes_list.append(
                f"Script overwrote challenge file {Path(path_str).name}; "
...
records.append({
    "type": "summary",
    "text": (
        f"Script execution {summary_state}: "
        f"exit code {returncode}, {len(flag_candidates)} flag candidate(s)."
    ),
})
records.append({
    "type": "finding",
    "finding_id": "finding-script-execution",
    "title": "LLM-generated script executed",
    "severity": result_severity,
    "description": (
        f"Executed LLM-generated {script_language} script. "
...
records.append({
    ...
    "manual_checks": [
        ...
        "If script failed, refine the approach and regenerate.",
    ],
})
