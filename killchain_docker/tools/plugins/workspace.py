...
def _generated_artifact_publish_function() -> str:
    return _join_shell(
        ...
        '  echo "$_kc_priority";',
        "};",
        "_kc_artifact_depth_score() {",
        '  _kc_rel="$1";',
        '  _kc_depth=$(printf "%s" "$_kc_rel" | awk -F/ \'{print NF - 1}\');',
        "  _kc_score=$((999 - _kc_depth));",
        '  if [ "$_kc_score" -lt 0 ]; then _kc_score=0; fi;',
        '  echo "$_kc_score";',
        "};",
        ...
        '  _kc_depth_score=$(_kc_artifact_depth_score "$_kc_rel");',
        '  _kc_size=$(stat -c%s "$_kc_src" 2>/dev/null || stat -f%z "$_kc_src" 2>/dev/null || echo 0);',
        "  _kc_size_kb=$(((_kc_size + 1023) / 1024));",
        '  printf "%03d\\t%03d\\t%08d\\t%s\\t%s\\t%s\\n" "$_kc_priority" "$_kc_depth_score" "$_kc_size_kb" "$_kc_src" "$_kc_origin" "$_kc_rel" >> "$_kc_candidates";',
        "};",
        "_kc_publish_generated_artifacts() {",
        ...
        "  fi;",
        "  while IFS=$'\\t' read -r _kc_priority _kc_depth_score _kc_size_key _kc_src _kc_origin _kc_rel; do",
        '    [ -n "$_kc_src" ] || continue;',
        '    _kc_publish_one_artifact "$_kc_src" "$_kc_origin" "$_kc_rel";',
        "  done < <(sort -t $'\\t' -k1,1nr -k2,2nr -k3,3n \"$_kc_candidates\" 2>/dev/null);",
