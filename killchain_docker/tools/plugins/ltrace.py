...
from killchain_docker.state.domain import Artifact, Credential, FlagCandidate
...
_CALL_RE = re.compile("^(?:\\[\\w+\\]\\s+)?(\\w+)\\(([^)]*)\\)\\s*=\\s*(.+)$")
_EXIT_RE = re.compile("\\+\\+\\+ exited \\(status (\\d+)\\) \\+\\+\\+")
...
def build_output(
            ...
            if func_name in _COMPARE_FUNCS and len(args) >= 2:
                ...
                if arg1 and arg2 and (len(arg1) >= 2) and (len(arg2) >= 2):
                    interesting_comparisons.append(
                        {"function": func_name, "arg1": arg1[:200], "arg2": arg2[:200]}
                    )
    ...
    summary = (
        f"ltrace {path}: {len(calls)} call(s), {len(call_counts)} unique function(s)"
    )
