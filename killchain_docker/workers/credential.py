...
from killchain_docker.reasoning import CredentialHarvestGuidance
    ...
    def run(self, task: Task, state: GlobalState) -> WorkerReport:
        if self.tool_gateway is None:
            return WorkerReport(
                ...
                error="CredentialHuntAgent.tool_gateway is None",
                retryable=False,
            )
        ...
        try:
            bundle = self.run_capability(
                task=task,
                capability=ToolCapability.CREDENTIAL_HARVEST,
        ...
        suggested_tasks = build_flag_validation_tasks(
            flag_candidates, source=ToolCapability.CREDENTIAL_HARVEST.value
        )
        suggested_tasks.extend(build_path_probe_tasks_for_assets(state, interesting_paths, priority=75))
        if credential_ids:
...
WORKER_CLASSES: tuple[type, ...] = (CredentialHuntAgent,)
