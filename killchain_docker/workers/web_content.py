    ...
    def run(self, task: Task, state: GlobalState) -> WorkerReport:
        ...
        if self.tool_gateway is None:
            return WorkerReport(
                ...
                error=(
                    "WebContentAgent.tool_gateway is None — "
                    f"configure capability {ToolCapability.HTTP_CONTENT.value!r} before dispatching web.content_review tasks"
                ),
                retryable=False,
            )
        ...
        potential_flags = []
        for candidate in (
            list(bundle.parsed.output_context.get("potential_flags") or [])
            + list(bundle.parsed.output_context.get("flag_candidates") or [])
            + list(note.potential_flags)
        ):
            text = str(candidate).strip()
            if text and text not in potential_flags:
        ...
        output_context: dict[str, Any] = {
            **bundle.parsed.output_context,
            ...
            "flag_candidates": potential_flags,
        }
        suggested_tasks = build_flag_validation_tasks(
            potential_flags, source="web_content"
        )
