...
- :func:`run_capability` - run a stable tool capability via the worker gateway
- :func:`evidence_review_guidance` - request grounded LLM synthesis for an evidence-heavy capability
...
def merge_review_outputs(
    ...
    *,
    ...
    """Merge LLM guidance into the capability output context."""
...
def success_report(
    *,
    ...
    output_context: dict[str, Any],
    planner_signals: list[PlannerSignal],
    notes: list[str],
    success: bool = True,
    error: str | None = None,
) -> WorkerReport:
...
def run_capability(
    ...
    *,
    ...
    """Convenience: returns ``(bundle, None)`` or ``(None, fail_report)``."""
    if worker.tool_gateway is None:
        return None, missing_execution_plane(
            worker_name=worker.name,
            task=task,
            hint=f"{type(worker).__name__}.tool_gateway is None - configure capability {capability.value!r}",
        )
    try:
        bundle = worker.run_capability(
            task=task,
