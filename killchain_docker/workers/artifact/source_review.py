...
from killchain_docker.workers._helpers.flag import extract_flag_candidates
...
from killchain_docker.state import GlobalState, Task, WorkerReport, looks_like_source
...
class SourceReviewAgent(WorkerAgent):
    ...
    def run(self, task: Task, state: GlobalState) -> WorkerReport:
        ...
        source_files = [path for path in input_files if looks_like_source(str(path))]
        ...
        if not source_files:
            return WorkerReport(
                ...
                success=True,
                summary="Source review skipped because no source-like files were provided.",
                output_context={
                    "source_files": [],
                    "skipped_files": skipped_files,
                    "skip_reason": "no_source_like_files",
                },
                notes=[
                    f"{self.name} skipped {len(skipped_files)} non-source file(s).",
                ],
                retryable=False,
            )

        capability = ToolCapability.ARTIFACT_SOURCE
        ...
        suggested_tasks.extend(build_path_probe_tasks_for_assets(state, interesting_routes))
