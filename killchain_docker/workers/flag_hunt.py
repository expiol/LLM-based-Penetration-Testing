...
from killchain_docker.prompts import get_worker_system_prompt, get_flag_hints
    ...
    def run(self, task: Task, state: GlobalState) -> WorkerReport:
        if self.tool_gateway is None:
            return WorkerReport(
                ...
                error="FlagHuntAgent.tool_gateway is None",
                retryable=False,
            )
        ...
        try:
            bundle = self.run_capability(
                task=task,
                capability=ToolCapability.FLAG_HARVEST,
        ...
        suggested_tasks = build_flag_validation_tasks(
            flag_candidates, source=ToolCapability.FLAG_HARVEST.value
        )
        suggested_tasks.extend(build_path_probe_tasks_for_assets(state, interesting_paths, priority=76))
