...
_HEX_BLOCK_RE = re.compile("(?:[0-9a-fA-F]{2}[\\s:]){4,}")
...
def _append_selected(
    item: tuple[float, int, EvidenceRecord],
    *,
...
class EvidenceContextBuilder:
    ...
    def build(
        self,
        state: RunState,
        allowed_capabilities: set | None = None,
        pinned_evidence_ids: list[str] | tuple[str, ...] | set[str] | None = None,
    ) -> list[dict[str, object]]:
        out: list[dict[str, object]] = []
        total_chars = 0
        pinned_ids = _text_set(pinned_evidence_ids)
        ...
        for rank, evidence in enumerate(selected):
            ...
            if tier_offset < 3:
                ...
                tier = "compressed"
            if evidence.evidence_id in pinned_ids:
                tier = "full"
            extracted = (
            ...
            if evidence.evidence_id in pinned_ids:
                item["selection_reason"] = "task_referenced"
            ...
            if len(item) > 5 or evidence.evidence_id in pinned_ids:
                item_chars = len(json.dumps(item, ensure_ascii=True))
                if (
                    out
                    and total_chars + item_chars > self.max_total_chars
                    and evidence.evidence_id not in pinned_ids
                ):
                    break
    ...
    def _select_records(
        ...
        allowed_values = {
            ...
            for cap in allowed_capabilities or set()
        }
        pinned_ids = pinned_evidence_ids or set()
        pinned: list[tuple[float, int, EvidenceRecord]] = []
        scored: list[tuple[float, int, EvidenceRecord]] = []
        records = EvidenceProjectionStore(state).records()
        total = max(1, len(records))
        for index, evidence in enumerate(records):
            is_pinned = evidence.evidence_id in pinned_ids
            if allowed_values and evidence.capability and not is_pinned:
                if evidence.capability not in allowed_values:
                    continue
            extracted = (
                evidence.extracted if isinstance(evidence.extracted, dict) else {}
            )
            ...
            item = (score + recency_bonus + near_miss_boost, index, evidence)
            if is_pinned:
                pinned.append(item)
                continue
            if score <= 0:
        ...
        for tool_name in ("script_exec", "shell_exec"):
            for _score, _index, evidence in sorted(
        ...
        selected: list[tuple[float, int, EvidenceRecord]] = []
        for item in sorted(pinned, key=lambda item: item[1]):
            _append_selected(item, selected=selected, selected_ids=selected_ids)
        ...
        return [
            ...
            for _score, _index, evidence in sorted(
                selected,
                key=lambda item: (
                    0 if item[2].evidence_id in pinned_ids else 1,
                    item[1],
                ),
            )
        ]
    ...
    def _score_record(self, evidence: EvidenceRecord, ctx: dict[str, object]) -> float:
        ...
        if tool_name == "script_exec":
            ...
            if ctx.get("failure_kind") not in (None, "", "none"):
                score += 4.0
            if any(
                (token in stdout.lower() for token in ("0x", "\\x", "hexdump", "xxd"))
            ) or _has_hex_block(stdout):
                score += 6.0
            if any(
                (
                    token in stdout.lower()
                    for token in ("disassembly", "objdump", "instruction")
                )
            ):
                score += 7.0
            if any(
                (token in stdout.lower() for token in ("uint", "int32", "byte", "bit"))
            ):
                score += 5.0
        ...
        if any(
            (
                token in stdout.lower()
                for token in ("disassembly", "objdump", "instruction", "sym.", "main")
            )
        ):
            score += 5.0
        if any(
            (
                token in stdout.lower()
                for token in ("uint", "int32", "byte", "bit", "xor", "shift", "lfsr")
            )
        ):
            score += 4.0
    ...
    def _script_context(
        self, ctx: dict[str, object], *, tier: str = "full"
    ) -> dict[str, object]:
        ...
        result: dict[str, object] = {
            ...
            "flag_candidates": _trim_list(
                ctx.get("flag_candidates"), limit=8, width=240
            ),
            "near_miss_candidates": _trim_list(
                ctx.get("near_miss_candidates"), limit=8, width=240
            ),
        ...
        if stderr:
            ...
            result["stderr_preview"] = self._trim_text(
                stderr, width=min(preview_width, 900)
            )
    ...
    def _generic_context(
        self, ctx: dict[str, object], *, evidence: EvidenceRecord, tier: str = "full"
    ) -> dict[str, object]:
        keep_keys = (
            "returncode",
            "path",
        ...
        if stderr:
            ...
            out["stderr_preview"] = self._trim_text(
                stderr, width=min(preview_width, 700)
            )
        return out
    ...
    def _key_lines(self, text: str) -> list[str]:
        ...
        if lines:
            return lines
        return [
            self._trim_text(line.strip(), width=260)
