"""Guardrails for script execution and flag plausibility."""
...
import killchain_docker.reasoning.flag as flag_module
...
from killchain_docker.state.constants import (
    plausible_flag,
    validatable_flag_candidate as is_validatable_flag_candidate,
)
...
import killchain_docker.tools.plugins._base as tool_base
import killchain_docker.tools.plugins.script_runtime as script_runtime
...
def _intent(capability: str, profile: str) -> dict[str, str]:
    return {"profile": profile, "required_capability": capability}
...
class _StaticDiskExtractPlugin:
    ...
    def execute(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        self.calls += 1
        path = "/home/ctfplayer/ctf_files/.autopentest_artifacts/disk_extract_out/offset_0/slides.pptx"
        stdout = "\n".join(
            [
...
class _StaticMediaScanPlugin:
    ...
    def execute(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        artifact = "/home/ctfplayer/ctf_files/.autopentest_artifacts/media_scan_out/image1.appended.bin"
        stdout = "\n".join(
            [
...
class TestValidatableFlagCandidate(unittest.TestCase):
    def test_accepts_canonical_flags(self) -> None:
        ...
        self.assertTrue(
            is_validatable_flag_candidate("key{And yes the nsa can read this to}")
        )
    ...
    def test_accepts_bare_token(self) -> None:
        self.assertTrue(
            is_validatable_flag_candidate("STFU_THIS_CHALLENGE_WAS_TOTALLY_NOT_LAME")
        )
    ...
    def test_rejects_low_information_repeated_bare_tokens(self) -> None:
        ...
        self.assertFalse(
            is_validatable_flag_candidate("TTTTTTTTTTTTTTTTTTTTTTTHHHHHHHHH")
        )
    ...
    def test_rejects_metadata_namespace_and_uuid_bare_tokens(self) -> None:
        self.assertFalse(
            is_validatable_flag_candidate("63A36ECB-BC33-4E88-9986-F84A006F35E4")
        )
    ...
    def test_does_not_reject_bare_token_only_because_it_has_a_suffix_like_segment(
        self,
    ) -> None:
    ...
    def test_rejects_format_placeholders(self) -> None:
        ...
        self.assertFalse(is_validatable_flag_candidate("flag{#=WlCj_B\\\\He}"))
...
class TestWorkerInnerLoopPolicy(unittest.TestCase):
    def test_script_rules_do_not_include_domain_specific_pcap_escape(self) -> None:
        rules = " ".join(tool_use_rules({ToolCapability.SCRIPT_EXEC})).lower()
    ...
    def test_script_failure_returns_to_planner_after_prior_diagnostic(self) -> None:
        ...
        self.assertFalse(
            should_continue_after_step(task, prior_steps, max_inner_steps=3)
        )

    def test_metadata_validation_followed_by_script_failure_allows_one_repair(
        self,
    ) -> None:
        task = TodoItem(goal="Recover the flag from computed plaintext.")
        prior_steps = [
    ...
    def test_metadata_validation_failure_kind_preserves_repair_signal(self) -> None:
        self.assertEqual(
            ToolGuardPolicy.metadata_failure_kind(
    ...
    def test_script_nonzero_exit_preserves_stdout_error_detail(self) -> None:
        request = ToolExecutionRequest(
            ...
            metadata={
                ...
                "script_code": "raise SystemExit(1)",
            },
            timeout_s=5,
        )
        ...
        self.assertIn(
            "Lines have different lengths", output.output_context["failure_detail"]
        )

    def test_metadata_validation_retries_once_then_hands_back_to_planner(self) -> None:
        calls = 0

        def invalid_script_response(
        ...
        result = worker.run(
            TodoItem(
                ...
                context={
                    ...
                    "dispatch_intent": _intent("artifact.triage", "artifact_analysis"),
                    "path": "/home/ctfplayer/ctf_files/out.bin",
                },
            ),
            RunState(objective="test"),
    ...
    def test_disk_extract_intent_runs_without_llm_selection(self) -> None:
        ...
        result = worker.run(
            TodoItem(
                ...
                context={
                    ...
                    "dispatch_intent": _intent("disk.extract", "container_extraction"),
    ...
    def test_office_inspect_intent_runs_without_llm_selection(self) -> None:
        ...
        result = worker.run(
            TodoItem(
                ...
                context={
                    "family": "artifact-followup",
                    "dispatch_intent": _intent("office.inspect", "office_inspection"),
    ...
    def test_media_scan_intent_runs_without_llm_selection(self) -> None:
        ...
        result = worker.run(
            TodoItem(
                ...
                context={
                    "family": "artifact-followup",
                    "dispatch_intent": _intent("media.scan", "media_inspection"),
    ...
    def test_png_inspect_intent_runs_without_llm_selection(self) -> None:
        ...
        result = worker.run(
            TodoItem(
                ...
                context={
                    "family": "artifact-followup",
                    "dispatch_intent": _intent("png.inspect", "image_inspection"),
...
class TestBracketSpanExtraction(unittest.TestCase):
    ...
    def test_script_stdout_accepts_bracket_candidate_with_key_context(self) -> None:
        text = "validated key plaintext: MY key for you is {And yes the nsa can read this to}\n"
        candidates = flag_candidates_from_script_stdout(text, source="script")
        self.assertIn(
    ...
    def test_script_stdout_rejects_negative_bracket_context(self) -> None:
        text = "no flag found in plaintext: {And yes the nsa can read this to}\n"
        candidates = flag_candidates_from_script_stdout(text, source="script")
...
class TestFlagRecoveryTaskDetection(unittest.TestCase):
    def test_does_not_treat_flag_named_artifact_parsing_as_flag_recovery(self) -> None:
        ...
        self.assertFalse(is_flag_recovery_task(todo))
    ...
    def test_detects_explicit_plaintext_or_flag_recovery(self) -> None:
        ...
        self.assertTrue(is_flag_recovery_task(todo))
    ...
    def test_flag_format_prefix_wins(self) -> None:
        ...
        candidates = _bracket_span_candidates(text, flag_format_prefix="key")
        self.assertEqual(candidates[0], "key{And yes the nsa can read this to}")
    ...
    def test_does_not_extract_metadata_namespace_or_uuid_tokens(self) -> None:
        text = "metadata: chromaticity com.adobe.xmp 22-rdf-syntax-ns 63A36ECB-BC33-4E88-9986-F84A006F35E4 CTF_TEMP_DIR"
        candidates = extract_flag_candidates(text)
        self.assertEqual(candidates, [])
    ...
    def test_does_not_lift_unlabeled_diagnostic_bare_tokens(self) -> None:
        ...
        self.assertEqual(
            extract_flag_candidates("lsb_rgb_4_msb.bin: hoyo%w_DOwF_F_5OVo|O`/%xofOUo"),
            [],
        )
    ...
    def test_extracts_repeated_letter_ascii_art_banner(self) -> None:
        text = "HHHHH     HHHHH EEEEEEEEE LLLLL     LLLLL OOOOOOOOO                         WWWWW     WWWWW OOOOOOOOO RRRRRRRR LLLLL DDDDDDDD                         OOOOOOOOO KKKKKKKK\n"
    ...
    def test_does_not_lift_key_diagnostic_fields_as_bare_candidates(self) -> None:
        self.assertEqual(
            extract_flag_candidates(
                "Key 'IHDR': starts_png=False, printable_in_first_200=113"
            ),
            [],
        )
        self.assertEqual(
    ...
    def test_does_not_lift_key_material_or_rejected_candidate_tokens(self) -> None:
        self.assertEqual(extract_flag_candidates("Derived key: b'WoAh_A_KWoAh'"), [])
        self.assertEqual(extract_flag_candidates("XOR key: WoAh_A_KWoAh"), [])
        self.assertEqual(
            extract_flag_candidates(
    ...
    def test_does_not_lift_source_code_identifier_tokens_from_key_context(self) -> None:
        text = "IV/nonce references: ['private $CIPHER = MCRYPT_RIJNDAEL_128;', \"private $key =  '13somerandomkey2';\", 'private $MODE = MCRYPT_MODE_ECB;']\nKey management references: ['$plaintext = mcrypt_decrypt($this->CIPHER, $this->key, $ciphertext, $this->MODE);']\n"
        self.assertEqual(extract_flag_candidates(text), [])
    ...
    def test_weak_context_does_not_lift_lowercase_labels(self) -> None:
        ...
        self.assertEqual(
            extract_flag_candidates("FLAG FOUND: autoCompressPictures"), []
        )
    ...
    def test_structured_error_span_is_not_wrapped_as_flag(self) -> None:
        text = "Results:\n{'command': './stfu flag.stfu', 'error': \"[Errno 2] No such file or directory: 'strace'\"}\n"
    ...
    def test_oversized_encoded_blob_is_not_decoded(self) -> None:
        ...
        try:
            self.assertEqual(extract_flag_candidates("A" * 200000), [])
        finally:
    ...
    def test_tool_flag_extraction_uses_bounded_scan_text(self) -> None:
        ...
        try:
            tool_base._flag_candidates_from("A" * 1000000)
        finally:
            tool_base.extract_flag_candidates = old_extract
        self.assertLessEqual(captured["length"], 170000)
    ...
    def test_tool_flag_extraction_keeps_relevant_middle_window(self) -> None:
        ...
        self.assertEqual(
            [item.value for item in candidates], ["flag{middle_window_candidate}"]
        )


class TestScriptOutputFailureSignals(unittest.TestCase):
    def _output_context(
        self, stderr: str = "", *, stdout: str = "", exit_code: int = 1
    ) -> dict[str, object]:
    ...
    def test_classifies_runtime_guard_reported_on_stdout(self) -> None:
        ctx = self._output_context(
            stdout="Round 1: work started\nError at round 1: script.exec Python time limit exceeded at line 166; use bounded loops or fast-forward math\n"
        )
...
class TestScriptExecutionRuntime(unittest.TestCase):
    def test_python_runtime_guard_tracks_request_timeout_by_default(self) -> None:
        old_limit = script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S
        script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S = 0
        try:
            wrapper = script_runtime.python_runtime_guard_wrapper(300)
        finally:
            script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S = old_limit
        self.assertIn("_kc_runtime_limit_s = 299", wrapper)

    def test_python_runtime_guard_can_be_capped_for_tests(self) -> None:
        old_limit = script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S
        script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S = 7
        try:
            wrapper = script_runtime.python_runtime_guard_wrapper(300)
        finally:
            script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S = old_limit
    ...
    def test_network_script_timeout_is_capped(self) -> None:
        ...
        self.assertEqual(
            captured["timeout_s"], script_runtime.NETWORK_SCRIPT_TIMEOUT_CAP_S
        )
    ...
    def test_python_busy_loop_hits_runtime_guard(self) -> None:
        plugin = ScriptPlugin()
        old_limit = script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S
        script_runtime.PYTHON_SCRIPT_RUNTIME_LIMIT_S = 1
        try:
            with tempfile.TemporaryDirectory() as tmp:
                result = plugin.execute(
                    ToolExecutionRequest(
    ...
    def test_success_without_flag_has_structured_no_candidate_signal(self) -> None:
        ...
        self.assertEqual(
            output.output_context["result_quality"], "partial_no_candidate"
        )
    ...
    def test_success_without_flag_keeps_guardrail_signal_from_output(self) -> None:
        output = build_script_output(
            ...
            ToolExecutionResult(
                ...
                mode=ExecutionMode.LOCAL_COMMAND,
                exit_code=0,
                stdout="Round 1 started\nError during interaction: script.exec Python time limit exceeded at line 64; use bounded loops or fast-forward math\nNo flag captured.\n",
    ...
    def test_successful_script_socket_timeout_diagnostic_is_not_tool_timeout(
        self,
    ) -> None:
        output = build_script_output(
            ToolExecutionRequest(
        ...
        self.assertEqual(output.output_context["failure_kind"], "no_candidate")
        self.assertNotEqual(
            output.output_context["partial_reason"],
            "script exceeded its execution or socket timeout",
        )
    ...
    def test_escaped_python_repr_does_not_emit_flag_candidates(self) -> None:
        output = build_script_output(
            ...
            ToolExecutionResult(
                ...
                mode=ExecutionMode.LOCAL_COMMAND,
                exit_code=0,
                stdout="First 200 bytes: b'\\x96\\\\\\\\\\x96{\\x9e\\x9e{\\r\\xff\\xff\\r6\\xcf\\xcf6|}'\n",
                stderr="",
            ),
            ParsedToolOutput(summary="raw"),
        )
    ...
    def test_source_identifier_tokens_do_not_survive_script_context_filter(
        self,
    ) -> None:
        stdout = "Key management references: [\"private $key =  '13somerandomkey2';\", '$plaintext = mcrypt_decrypt($this->CIPHER, $this->key, $ciphertext, $this->MODE);']\nIV/nonce references: ['private $CIPHER = MCRYPT_RIJNDAEL_128;', 'private $MODE = MCRYPT_MODE_ECB;']\n"
        output = build_script_output(
            ToolExecutionRequest(
                tool_name="script_exec", metadata={"script_language": "python"}
            ),
    ...
    def test_derived_ascii_art_candidate_survives_script_context_filter(self) -> None:
        line = "HHHHH     HHHHH EEEEEEEEE LLLLL     LLLLL OOOOOOOOO                         WWWWW     WWWWW OOOOOOOOO RRRRRRRR LLLLL DDDDDDDD                         OOOOOOOOO KKKKKKKK"
        stdout = "\n".join([line, line, line])
        output = build_script_output(
            ToolExecutionRequest(
                tool_name="script_exec", metadata={"script_language": "python"}
