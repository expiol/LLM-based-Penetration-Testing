...
def test_pipeline_dvwa() -> None:
    if which("nmap") is None or which("sqlmap") is None:
        pytest.skip("nmap or sqlmap not installed; run in lab container")
    if not _lab_ready("http://127.0.0.1:8080"):
        pytest.skip("DVWA not reachable; run scripts/lab_up.sh")

    result = run_assessment(
        target_path=Path("data/targets/dvwa_local.yaml"),
        config_path=Path("configs/dev.yaml"),
        authorized=True,
        success_strategy="dvwa_sqlmap",
    )
