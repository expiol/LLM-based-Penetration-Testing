...
class ToolMetadataContractTests(unittest.TestCase):
    ...
    def test_artifact_triage_context_path_overrides_challenge_file_defaults(self) -> None:
        state = RunState(
            objective="solve",
            metadata={"challenge": {"files": ["sleeping_dist.py"]}},
        )
    ...
    def test_shell_exec_rejects_complex_python_one_liner(self) -> None:
        ...
        todo = TodoItem(goal="test")
        command = (
            "python3 -c \"import socket; data=b''; "
            "while True: chunk=sock.recv(4096); data += chunk\""
        )
            ...
            normalize_tool_metadata(
                ...
                state,
                {
                    "command": (
                        ...
                        "tcp://example:31337 2>&1 || echo CURL_FAILED"
                    ),
                },
            )
    ...
    def test_path_cli_tool_resolves_relative_path_under_files_root(self) -> None:
        ...
        todo = TodoItem(goal="test")
        result = normalize_tool_metadata(
            ToolCapability.FILE_CMD, todo, state,
            {"path": "stfu"},
        )
    ...
    def test_path_cli_tool_respects_context_files_root(self) -> None:
        ...
        result = normalize_tool_metadata(
            ToolCapability.STRINGS_CMD, todo, state,
            {"path": "./payload.bin"},
        )
